# GREAT MINDS — Implementation Backlog v15

> **Purpose:** Source of truth for implementation work for Great Minds / Philosopher v1 launch.
> **v15 = v14 baseline (2026-05-30) + 2026-06-01 session delta (Mirror feature shipped end-to-end; PRs #166–#173; migrations 017 + 018; host picker live; MONTAGE FIDELITY resolved via "said=kernel" rule).**
>
> **Generated:** 2026-06-01 (v15 rotation)
>
> **How to read this file:**
> - This v15 file supersedes v14 and all prior backlog files.
> - Where v15 conflicts with v14, v15 wins.
> - Status, priority, and launch-readiness calls reflect 2026-06-01 state.
>
> **Companion documents:**
> - `PROJECT_STATE_v15.md` — current project state
> - `HANDOFF_BRIEF_v15.md` — continuity and implementation history
...
**Shipped this session (all squash-merged to main, PRs #166–#173):**

- **The Mirror** — weekly "said → meant" reflection artifact, end-to-end live & verified. Generator, idempotent cron (weekly + preview), eligible-host config, ring-true feedback, host picker UI. Migrations 017 + 018.

**Resolved:**
- **MONTAGE FIDELITY** — ✅ RESOLVED. The `said = charged kernel (one line)` rule in MIRROR_PROMPT handles the montage-fidelity concern: the Mirror always works from a single distilled kernel of what the user said, not a summary or montage. No separate implementation needed.

**Status:** Mirror ✅ COMPLETE. Critical path to revenue: TD-11 → disable `BETA_GRANT_PRO_TO_ALL` → Stripe checkout smoke test (with BETA flag OFF). Do not start retention features before revenue is validated.
...
- [x] ~~**The Mirror**~~ — SHIPPED (2026-06-01). Mirror sub-feature of Rituals complete.
- [ ] **Rituals guided programs** — Counterview + remaining guided programs; scope NOT YET DESIGNED. First step: design session with Claude (chat). User need: purpose/progress ("am I improving?", "what's my trajectory?").
...
19. ~~**The Mirror**~~ ✅ DONE (2026-06-01) — PRs #166–#173; generator + idempotent cron + host picker + ring-true; migrations 017 + 018; eligible-host config locked; end-to-end verified
20. **TD-11 — Tier resolution refactor** — required before BETA flag can be safely disabled [REVENUE GATE]
21. **Disable BETA_GRANT_PRO_TO_ALL** — then run end-to-end Stripe sandbox test [REVENUE GATE]
22. **End-to-end Stripe sandbox test** (with BETA flag OFF)
23. **source_chunks re-ingest** into Oregon (TD-22; status unconfirmed post-switch)
24. **Post-Oregon smoke test** — login, chat, Mirror, share, library, RAG retrieval
25. **Mobile 12-point nav smoke test**
26. **Cold beta with 3–5 fresh users**
27. **Cold validation with external users** (retention + willingness-to-pay)
28. **Rituals guided programs** — Counterview + guided programs; scope NOT YET DESIGNED; design session first
...
- [ ] **Post-Oregon smoke test** (login, chat, Mirror, rituals, share, library, RAG retrieval)
...
See `PROJECT_STATE_v15.md` §4. Migration head: `018_user_mirror_host`. Migrations 017 (mirrors table) and 018 (users.mirror_host_slug) added 2026-06-01.

---

## 5. Config & Environment Variables

See `HANDOFF_BRIEF_v15.md` §7 for full env var list.
...
- "Take it to the Council" in Mirror = locked teaser CTA only

### 9.8 Mirror fast-follows (post-first-paying-user; NOT before)

These should NOT block launch or the first Stripe test. Implement after first paying user is confirmed.

- [ ] **Mirror v2 — branded email postcard** — host's closing words + thumbnail + date; reuse Resend infra. Delivery: Sunday or next-day-after-generation. P1.
- [ ] **Host-aware handoff** — "Continue with {host}" CTA at Mirror closing; reuse existing `CROSS_MIND_NOTE` pattern (`source_persona_slug` / `source_context_content` schema already supports this). No new migrations needed. P1.
- [ ] **Smart input cap on Mirror generator** — cost control at scale; intelligent summarisation of user input before LLM call, never crude truncation. Implement before user count grows past cold beta. P1.
- [ ] **B5/B6 spec drift** — portrait-first hero vs spec card layout mismatch; P2 cosmetic polish only.

### 9.9 Counterview — TODO: spec §1.3.2 exists; implementation NOT DESIGNED

Counterview is described in the Option B ritual lock and spec §1.3.2 (Setup → 2 rounds steelman-the-opposite → 2-line closing "What shifted, what didn't"). The flow is locked but the implementation has not been designed.

**Do not dispatch a build brief until a design session is complete.**

- Proposed host: Machiavelli (Pro-only, no free preview)
- Depends on Mirror being stable in production first
...
- [x] ~~**The Mirror**~~ ✅ DONE (2026-06-01) — PRs #166–#173; generator + cron + host picker + ring-true; migrations 017 + 018; end-to-end verified
...
**End of IMPLEMENTATION_BACKLOG v15.** Authoritative as of 2026-06-01. Supersedes `IMPLEMENTATION_BACKLOG_v14.md` (preserved as historical reference).

# HANDOFF BRIEF v19 — Philosopher / The Wise Room

**For:** The next Claude (chat) and Claude Code session
**From:** Nikos Koutras (founder) + Claude Code
**Date updated:** 2026-06-16
**Prior version:** `docs/HANDOFF_BRIEF_v18.md` (2026-06-15)
**Generated:** 2026-06-16 (v19 rotation)

**Block trigger for v19 rotation:** Persona-roster + asset block landed since the v18 doc (which stopped at PR #313) — portraits standardized to WebP (migration 026) and two new pro-tier personas, George Orwell + Miyamoto Musashi, wired end-to-end (migration 027). Migration head moved 025 → 027; roster moved 9 → 11.

**v19 summary (2026-06-16):** Portrait WebP standardization ✅ (#315 — all personas 1024px/q82, both stores, guide `src`s updated; migration 026 repoints the 6 non-webp `portrait_url`s). George Orwell + Miyamoto Musashi ✅ (#316 — pro-tier, active; persona modules + brain YAML; matching affinities; migration 027 self-contained with immutable inline config snapshot). RAG: Orwell copyright-excluded (zero chunks); Musashi deferred — absent from `CORPUS_SOURCES` pending a rights-clean translation. Council roster unchanged. Current main: `0a42b0cb`.
...
- **Portrait WebP standardization ✅ SHIPPED (2026-06-16, #315)** — all personas 1024px/q82 WebP in both stores; migration 026 repoints the 6 non-webp `portrait_url`s
- **George Orwell + Miyamoto Musashi ✅ SHIPPED (2026-06-16, #316)** — 2 new pro-tier personas (roster 9 → 11); migration 027; Orwell copyright-excluded from RAG, Musashi RAG deferred

> **v19 conflict resolution rule:** Where v19 conflicts with v18 or earlier, v19 wins. Production reality always wins over docs.

---

## 2026-06-16 Session Delta — WebP portrait standard · George Orwell + Miyamoto Musashi (roster 9 → 11)

> Appended as v19. Where this conflicts with earlier sections, this section wins. **Current main SHA: `0a42b0cb` (PR #316).**

**Portrait assets → WebP standard (#315):**
- All persona portraits are now **WebP, 1024px, quality 82**. New oil-painting portraits delivered; remaining non-webp assets converted so the whole roster is uniform.
- **Two asset stores, kept in sync:** `apps/web/public/personas/<slug>.webp` and `apps/api/static/personas/<slug>.webp`. Any new portrait ships `.webp` to **both**.
- **Migration 026 (`026_personas_portrait_webp`)** repointed the **6** previously non-webp DB `portrait_url`s (carl_jung, lao_tzu, machiavelli, oscar_wilde were `.png`; marcus_aurelius, socrates were `.jpg`) → `/personas/<slug>.webp`. Epictetus/Freud/de Beauvoir were already `.webp`.
- Guide screen `src`s updated (`apps/web/app/app/guide/page.tsx`).

**George Orwell + Miyamoto Musashi (#316) — roster 9 → 11:**
- Both **`tier=pro`, `is_active=true`**. Persona modules `personas/george_orwell.py` + `personas/miyamoto_musashi.py` (PersonaConfig from the v0.9.1 design YAML, authored in the `niccolo_machiavelli.py` style); brain YAML `philosopher_brain/personas/orwell.yaml` + `musashi.yaml`; registered in `PERSONA_REGISTRY`.
- **Matching:** `services/matching_service.py` `PERSONA_AFFINITIES` weights for both (all 12 themes + 4 needs); `EXCLUDED_SLUGS` unchanged. Smoke: decision+challenge → Musashi; controversy/self-deception+challenge → Orwell.
- **RAG / corpus:** `george_orwell` added to `EXCLUDED_PERSONAS` (`scripts/corpus_sources.py`) — US copyright; **voice-engineered only, zero chunks**. `miyamoto_musashi` is **NOT** excluded but **absent from `CORPUS_SOURCES`** (deferred until a rights-clean public-domain translation is sourced); zero chunks for now. `retrieval_sources=[]` for both.
- **Migration 027 (`027_add_orwell_musashi`, down_revision `026_personas_portrait_webp`):** self-contained data migration — **no app-code import**; `config` jsonb = **immutable inline literal snapshot** (verified identical to `to_dict()` at authoring, then frozen); `bio = about_en` verbatim; `portrait_url = /personas/<slug>.webp`; `is_active=true`; `tier=pro`. DOWN deletes the 2 rows by slug.
- **Migration head now `027_add_orwell_musashi`** (chain …025 → 026 → 027). 026/027 are data/asset migrations — no schema change.
- **Error voice:** neither new persona defines `error_messages` in its config snapshot, so `get_error_voice` (`services/persona_voice.py`) uses the **generic** `llm_unavailable` fallback for them (not a per-persona voice). Adding per-persona error voice is a possible follow-up.

**Deferred / known:**
- ⏸ **Musashi RAG ingestion** — pending rights-clean public-domain translation.
- ⏸ **Council default-panel rotation for the new personas** — optional product call; NOT done. Council roster unchanged (Machiavelli, Epictetus, Freud, de Beauvoir).
- The 9 pre-existing personas are otherwise untouched.

**New conventions logged (see `CLAUDE.md`):** (a) self-contained migrations (immutable inline config snapshot — the 027 pattern); (b) WebP portrait asset standard, both stores in sync; (c) "Adding a persona" checklist.

...
async def _persona_id(db, slug: str) -> str:
    ...
    str(), and it is load-bearing rather than tidiness. asyncpg returns a UUID
    column as uuid.UUID, while the ORM maps these columns with as_uuid=False so
    Conversation.persona_id is a plain str. Comparing the two gives
    `'f845…' == UUID('f845…')` — False on identical values, which reads as a
    product bug and is not one. Coerced at this boundary, the way
    test_memory_recall_and_cascades.py:94 does it, so every id in this file is a
    str and the annotation above is true.
    """

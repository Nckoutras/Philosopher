...
def setup_cron(arq_queue):
    ...
    # weekly_letter and monthly_letter USED TO BE HERE. They are now ARQ cron
    # jobs in workers/letter_dispatch.py, registered on
    # WorkerSettings.cron_jobs, so they run in the worker process and write a
    # job_run row per run (R4). The five jobs left in this file are unchanged
    # and still run in the API process; the other three dispatch jobs move
    # later, in one PR, only after a Sunday run proves the pattern.

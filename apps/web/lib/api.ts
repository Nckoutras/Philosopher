...
class ApiClient {
  ...
  async streamCouncil(body: { matter: string; source?: string; mirror_id?: string | null; insight_id?: string | null; conversation_id?: string | null; matter_edited?: boolean }): Promise<Response> {
  ...
  async shareQuote(quoteId: string): Promise<Blob> {
    const res = await fetch(`${API_BASE}/share/quote`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...(this.token ? { Authorization: `Bearer ${this.token}` } : {}),
